...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_no_moves_once_winner(self):
        '''Check if any more moves are registered after there is a winner. '''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        self.assertTileIs(tiles[0], self.SYMBOL_BLANK)
        tiles[0].click()
    ...
    def test_no_new_pieces_after_three(self):
        '''Check that when a board has three X's already, the next X will delete an existing piece (the first click), and add a new piece in an empty square'''
        '''Also check that if the second click is on a preoccupied square, no moves will be taken and click count will reset to 0 '''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()
        tiles[1].click()
        tiles[2].click()
    ...
    def test_middle_square(self):
        '''Check that if there are three pieces on the board, and one is in the middle square, then the next move will not work if it doesn't vacate the square or win'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()
        tiles[1].click()
        tiles[4].click()
